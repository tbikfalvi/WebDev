...
if ($userid > 1)
{
	$hasz_pos_id	= request_var('HASZP', 0);
	$hasz_neg_id	= request_var('HASZN', 0);
	if ($hasz_pos_id > 1)
	{
		$sql = "SELECT count(*) cnt FROM jatekszervezes_Hasznos_szav WHERE user_id=$userid AND post_id=$hasz_pos_id"; 
		$result = $db->sql_query($sql);
		$hassz_exist = (int) $db->sql_fetchfield('cnt');
		$db->sql_freeresult($result);
		if ($hassz_exist==0) 
		{
			$sql="INSERT INTO jatekszervezes_Hasznos_szav(user_id, post_id) values ($userid,$hasz_pos_id)"; 
			$db->sql_query($sql);
			$sql = "UPDATE  jatekszervezes_hasznos SET ertek=ertek + 1 WHERE post_id=$hasz_pos_id ";
			$db->sql_query($sql);
		}
		redirect(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "p=$hasz_pos_id#p$hasz_pos_id"));
	}
	if ($hasz_neg_id > 1)
	{
		$sql = "SELECT count(*) cnt FROM jatekszervezes_Nem_hasznos_szav WHERE user_id=$userid AND post_id=$hasz_neg_id"; 
		$result = $db->sql_query($sql);
		$hassz_exist = (int) $db->sql_fetchfield('cnt');
		$db->sql_freeresult($result);
		if ($hassz_exist==0) 
		{
			$sql="INSERT INTO jatekszervezes_Nem_hasznos_szav(user_id, post_id) values ($userid,$hasz_neg_id)"; 
			$db->sql_query($sql);
			$sql = "UPDATE  jatekszervezes_hasznos SET ertek=ertek - 1 WHERE post_id=$hasz_neg_id ";
			$db->sql_query($sql);
		}
		redirect(append_sid("{$phpbb_root_path}viewtopic.$phpEx", "p=$hasz_neg_id#p$hasz_neg_id"));
	}
}
//  hasznos //
...
$hp_del_id	= request_var('HPDID', 0);
$curr_forum_date= time();

if ($hp_up_id > 1)
{
	$hp_past_time = $curr_forum_date-5184000;
	$sql="UPDATE " . POSTS_TABLE . " SET post_time = $curr_forum_date WHERE post_id = $hp_up_id AND post_time <= $hp_past_time AND topic_id = $topic_id AND forum_id=97 ";
	//AND poster_id=$userid "; 
	$db->sql_query($sql);
	$sql = "UPDATE " . TOPICS_TABLE . " SET topic_last_post_id=$hp_up_id, topic_last_post_time=$curr_forum_date WHERE topic_id=$topic_id AND forum_id=97 ";
	$db->sql_query($sql);
}
...
if ($hp_del_id > 1)
{
	switch ($topic_id)
	{
		case 1530: $new_topic_id = 3416; break; 
		case 1534: $new_topic_id = 3409; break;
		case 1535: $new_topic_id = 3410; break;
		case 1539: $new_topic_id = 3407; break;
		case 1540: $new_topic_id = 3418; break;
		case 1541: $new_topic_id = 3413; break;
		case 1543: $new_topic_id = 3411; break;
		case 1545: $new_topic_id = 3412; break;
		case 1548: $new_topic_id = 3408; break;
		case 1549: $new_topic_id = 3417; break;
		case 1865: $new_topic_id = 3415; break;
		default: $new_topic_id  = 0; break;
	}
	if ($new_topic_id > 0)
	{
		$sql="UPDATE " . POSTS_TABLE . " SET topic_id = $new_topic_id WHERE post_id = $hp_del_id AND topic_id = $topic_id AND forum_id=97 ";
		// AND poster_id = $userid" ; 
		$db->sql_query($sql);
		$sql = "UPDATE " . TOPICS_TABLE . " SET topic_replies_real=topic_replies_real+1, topic_replies = topic_replies+1  WHERE topic_id=$new_topic_id ";
		$db->sql_query($sql);
		$sql = "UPDATE " . TOPICS_TABLE . " SET topic_replies_real=topic_replies_real-1, topic_replies = topic_replies-1  WHERE topic_id=$topic_id AND forum_id=97";
		$db->sql_query($sql);
	}
}
if (($hp_del_id > 1) or ($hp_up_id > 1))
{
	$hp_past_del_time = time()-6048000;
	$sql = " SELECT post_id, topic_id, poster_id FROM " . POSTS_TABLE . " p WHERE forum_id = 97 AND post_time <= $hp_past_del_time ";
	$result = $db->sql_query($sql);
	while ($row = $db->sql_fetchrow($result))
	{
		$had_topic_title	= $row['topic_title'];
		$had_poster_id		= $row['poster_id'];
		$had_post_id		= $row['post_id'];
	}
	$db->sql_freeresult($result);	
}
...
// szervezés változók behívása (sok van)
...
if ($jsz == 10)
{
	...
	$fee			= request_var('FEE',0);
	$szervezo		= $db->sql_escape(utf8_normalize_nfc(request_var('SZERVEZO','',true)));
	$kontakt		= $db->sql_escape(utf8_normalize_nfc(request_var('KONTAKT','',true)));
	$minimum		= request_var('MINIMUM',0);
	$gateopen		= $db->sql_escape(utf8_normalize_nfc(request_var('GATEOPEN','',true)));
	$gamestart		= $db->sql_escape(utf8_normalize_nfc(request_var('GAMESTART','',true)));
	$gameend		= $db->sql_escape(utf8_normalize_nfc(request_var('GAMEEND','',true)));
	$rules			= $db->sql_escape(utf8_normalize_nfc(request_var('RULES','',true)));
	$kotelezo		= $db->sql_escape(utf8_normalize_nfc(request_var('KOTELEZO','',true)));
	$ajanlott		= $db->sql_escape(utf8_normalize_nfc(request_var('AJANLOTT','',true)));
	$imagelink		= $db->sql_escape(utf8_normalize_nfc(request_var('IMAGELINK','',true)));
	$battlefield	= $db->sql_escape(utf8_normalize_nfc(request_var('BATTLEFIELD','',true)));
	$maplink		= $db->sql_escape(utf8_normalize_nfc(request_var('MAPLINK','',true)));

	$sql = "select count(*) cnt from jatekszervezes_Szervez where topic_id=$topic_id"; 
	$result = $db->sql_query($sql);
	...
	else
	{
		if ($gamedate == '')
		{
			$sql = "DELETE FROM jatekszervezes_Szervez WHERE topic_id=$topic_id"; 
			$db->sql_query($sql);
			
			$sql = "DELETE FROM jatekszervezes_Sides WHERE topic_id=$topic_id"; 
			$db->sql_query($sql);
			
			$sql = "DELETE FROM jatekszervezes_jelentk WHERE topic_id=$topic_id"; 
			$db->sql_query($sql);
		}
		else
		{
			$sql = "UPDATE jatekszervezes_Szervez SET 
				GameDate='$gamedate',GameEndDate='$gameenddate',Fee=$fee,BattleField='$battlefield',
				szervezo='$szervezo', kontakt='$kontakt', gateopen='$gateopen',
				gamestart='$gamestart', gameend='$gameend', rules='$rules',
				imagelink='$imagelink', gps='$gps', maplink='$maplink',
				minimum=$minimum, kotelezo='$kotelezo', ajanlott='$ajanlott'
				WHERE topic_id=$topic_id"; 
			$db->sql_query($sql);
		}
	}
...
if ($jsz == 11)
{
	...
	$sql = " SELECT (count(j.user_id)+sum(extra_player)) sum_jel FROM jatekszervezes_jelentk j, jatekszervezes_Sides s WHERE j.side=s.side_id and s.topic_id=$topic_id and s.side_player>0 and j.status=1";
	$result = $db->sql_query($sql);
	$sum_jel = (int) $db->sql_fetchfield('sum_jel');
	$db->sql_freeresult($result);

	$sql = "UPDATE jatekszervezes_Szervez SET PlayerNr=$sum_player, jelplayernr=$sum_jel WHERE topic_id=$topic_id "; 
	$db->sql_query($sql);
...
if ($jsz == 12)
{
	...
	for ($i = 0; $i < 20; $i++)
	{
		$modside_id[$i] = request_var('MODSIDE_ID'.$i,0);
		$modside[$i] = $db->sql_escape(utf8_normalize_nfc(request_var('MODSIDE'.$i,'',true)));
		$modside_nr[$i] = request_var('MODSIDE_NR'.$i,0);
		if ( $modside_id[$i] != 0)
		{	
			$modsize	= $i;
		}
	...
	for ($i = 0; $i <= $modsize; $i++)
	{
		if ( $modside[$i] == '' )
		{
			$sql = "DELETE FROM jatekszervezes_jelentk WHERE side=$modside_id[$i]"; 	
			$db->sql_query($sql);
			$sql = "DELETE FROM jatekszervezes_Sides WHERE side_id=$modside_id[$i]"; 	
		}
		else	
		{
			$sql = "UPDATE jatekszervezes_Sides SET side_name='$modside[$i]',  side_player='$modside_nr[$i]' WHERE side_id=$modside_id[$i]"; 
		}
		$db->sql_query($sql);
	}
...
if ($jsz == 20)
{
	...
	$lockside_id = array();
	$lockside_hideid = array();
	for ($i = 0; $i < 20; $i++)
	{
		
		$lockside_id[$i] = request_var('LOCKSIDE'.$i,0);
		$lockside_hideid[$i] = request_var('LOCKSIDEHIDE'.$i,0);
		if ( $lockside_hideid[$i] != 0)
		{	
			$locksize	= $i;
		}
	...
	// kontrolhoz kijelöl userek
	$sel_user = array();
	for ($i = 0; $i < 500; $i++)
	{
		$sel_user_base = request_var('SEL_USER_'.$i,0);
		if ($sel_user_base > 0)
		{
		  $sel_user[] = $sel_user_base; 
		}
	}

	$kontroll		= request_var('KONTROLL', 0);
	$kside			= request_var('K_SIDE',0);
	$kplatoon		= $db->sql_escape(utf8_normalize_nfc(request_var('K_PLATOON','',true)));
	...
	for ($i = 0; $i <= $locksize; $i++)
	{
		...
		{
			$sql = "UPDATE jatekszervezes_Sides SET side_open='Y' WHERE side_id=$lockside_hideid[$i]"; 	
			$db->sql_query($sql);
		}
		else	
		{
			$sql = "UPDATE jatekszervezes_Sides SET side_open='N' WHERE side_id=$lockside_hideid[$i]"; 	
			$db->sql_query($sql);
		}
	}

	// kijelöltek adott oldalra helyezése
	if ($kontroll == 1)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "UPDATE jatekszervezes_jelentk SET side=$kside WHERE user_id = $sel_user[$i] and topic_id=$topic_id"; 	
			$db->sql_query($sql);
		}
	}

	// kijelöltek adott rajba
	if ($kontroll == 2)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "UPDATE jatekszervezes_jelentk SET platoon='$kplatoon' WHERE user_id = $sel_user[$i] and topic_id=$topic_id"; 	
			$db->sql_query($sql);
		}
	}
	...
	if ($kontroll == 3)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "DELETE FROM jatekszervezes_jelentk WHERE user_id = $sel_user[$i] and topic_id=$topic_id"; 	
			$db->sql_query($sql);
		}
	}


	// kijelöltek jelentkezésének elfogadása
	if ($kontroll == 4)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "UPDATE jatekszervezes_jelentk SET reg_accept='Y' WHERE user_id = $sel_user[$i] and topic_id=$topic_id"; 	
			$db->sql_query($sql);
		}
	}

	// kijelöltek jelentkezésének elutasítása
	if ($kontroll == 5)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "UPDATE jatekszervezes_jelentk SET reg_accept='N' WHERE user_id = $sel_user[$i] and topic_id=$topic_id"; 	
			$db->sql_query($sql);
		}
	}
	...
	if ($kontroll == 6)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "INSERT INTO jatekszervezes_feketelista(topic_id,user_id,sender_id,add_date,remark)
					values (".$topic_id.",".$sel_user[$i].",".$user->data['user_id'].",curdate(),'".$feketeremark."')";
			$db->sql_query($sql);
		}
	}
	...
	if ($kontroll == 7)
	{
		for ($i = 0, $size = sizeof($sel_user); $i < $size; $i++)
		{
			$sql = "INSERT INTO jatekszervezes_feherlista(topic_id,user_id,sender_id,add_date,remark)
					values (".$topic_id.",".$sel_user[$i].",".$user->data['user_id'].",curdate(),'".$feherremark."')";
			$db->sql_query($sql);
		}
	}
	...
	unset($feherremark);
	unset($lockside_id);
	unset($lockside_hideid);
}


// Szervezés törzsadatok behívása db-ből
...
$sum_player = $row['PlayerNr'];	
$sum_jel	= $row['jelplayernr'];		
$fee		= $row['Fee'];			
$battlefield= $row['BattleField'];
$szervezo	= $row['szervezo'];
$kontakt	= $row['kontakt'];
...
$maplink	= $row['maplink'];
$minimum	= $row['minimum'];
$kotelezo	= $row['kotelezo'];
$ajanlott	= $row['ajanlott'];
...
if ($jsz == 30)
{
	...
	// jelentkezési adatok feltöltése inputból
	...
	$fps_s			= request_var('FPS_S',0);
	$transport 		= request_var('TRANSPORT',0);
	$tfrom 			= $db->sql_escape(utf8_normalize_nfc(request_var('TFROM','',true)));
	$tcap 			= request_var('TCAP',0);
	$team 			= $db->sql_escape(utf8_normalize_nfc(request_var('TEAM','',true)));
	$extra_pl 		= request_var('EXTRA_PL',0);
	$remark 		= $db->sql_escape(utf8_normalize_nfc(request_var('REMARK','',true)));
	$side 			= request_var('SIDE',0);
...
/*	
	if ($pd_req == 1 )
	{
		$sql = "UPDATE User_settings SET weap_m='$weap_m', fps_m='$fps_m', weap_s='$weap_s', fps_s='$fps_s',				
				transport='$transport', tfrom='$tfrom', tcap='$tcap', team='$team', 			
				pd_fname='$fname', pd_lname='$lname',pd_address='$address',pd_city='$city',pd_zip='$zip',
				pd_idn='$idn',pd_email='$email',pd_other='$pd_other' 
	...
	*/
	...
	if ($szervezett > 0)
	{
		$sql = "select if(side_open = 'Y', 1 , 0) side_open from jatekszervezes_Sides where topic_id=$topic_id and side_id=$side"; 
		$result = $db->sql_query($sql);
		$sopen = (int) $db->sql_fetchfield('side_open');
		$db->sql_freeresult($result);
		if ($sopen == 1)
		{
			$sql = "select count(*) cnt from jatekszervezes_jelentk where topic_id=$topic_id and user_id=$userid"; 
			$result = $db->sql_query($sql);
			$jelentkezett = (int) $db->sql_fetchfield('cnt');
			$db->sql_freeresult($result);
			if ($jelentkezett == 0)
			{
				if ($userid > 1)
				{
					$sql = "INSERT INTO jatekszervezes_jelentk (topic_id, user_id, weap_m, fps_m, weap_s, fps_s, side, transport, tfrom, tcap, team, 				extra_player, remark, status, jdate, sort) 
						VALUES ('$topic_id','$userid', '$weap_m', '$fps_m', '$weap_s', '$fps_s', '$side', '$transport', '$tfrom', '$tcap', '$team', '$extra_pl', '$remark', '$status', CURDATE(), '$sort')"; 
					$db->sql_query($sql);
				}
			}
			else
			{
				if (  substr($jel_refresh,0,6) ==  'Adatok' )
				{
					$sql = "UPDATE jatekszervezes_jelentk SET weap_m='$weap_m', fps_m='$fps_m', weap_s='$weap_s', fps_s='$fps_s', side='$side', 				transport='$transport', tfrom='$tfrom', tcap='$tcap', team='$team', extra_player='$extra_pl', 						remark='$remark', sort='$sort'  
						WHERE topic_id=$topic_id and user_id=$userid "; 
					$db->sql_query($sql);
				}
				else
				{
					$sql = "UPDATE jatekszervezes_jelentk SET weap_m='$weap_m', fps_m='$fps_m', weap_s='$weap_s', fps_s='$fps_s', side='$side', 				transport='$transport', tfrom='$tfrom', tcap='$tcap', team='$team', extra_player='$extra_pl', 						remark='$remark', status='$status', sort='$sort', jdate=CURDATE()  
						WHERE topic_id=$topic_id and user_id=$userid "; 
					$db->sql_query($sql);
				}
			}
		}
		//össz jázékoszám meg hányan jelentkeztek		
		$sql = " SELECT sum(side_player) sum_player FROM jatekszervezes_Sides WHERE topic_id=$topic_id and side_player>0";
		$result = $db->sql_query($sql);
		$sum_player = (int) $db->sql_fetchfield('sum_player');
		$db->sql_freeresult($result);
...
// jelentkezési adatok feltöltése db-ből
...
$us_lat		=  $row['pf_longt'];
$us_longt	=  $row['pf_lat'];
$trasmod	= $row['pf_minimalmod'];
if ($signup_id == '' )
{
	$weap_m		= $row['pf_elodleges'];
	$fps_m		= $row['pf_elodleges_fps'];
	$weap_s		= $row['pf_masodlagos'];
	$fps_s		= $row['pf_masodlagos_fps'];
	$team		= $row['pf_csapattagsag'];
}
$db->sql_freeresult($result);
if ($trasmod == 1 )
{
...
//lezárás, fekete/fehér lista megnyitása

$sql = "select if(GameDate-1 <= CURDATE(),'DISABLED','') lezaras from jatekszervezes_Szervez where topic_id=$topic_id"; 
$result = $db->sql_query($sql);
$lezaras = $db->sql_fetchfield('lezaras');
$db->sql_freeresult($result);

$sql = "select if(GameDate > CURDATE(),'DISABLED','') pontosztas from jatekszervezes_Szervez where topic_id=$topic_id"; 
$result = $db->sql_query($sql);
$pontosztas = $db->sql_fetchfield('pontosztas');
$db->sql_freeresult($result);

// jelentkeztés megjelenítése

$sql = " SELECT sum(status) status FROM jatekszervezes_jelentk WHERE topic_id=$topic_id and user_id=".$user->data['user_id'];
$result = $db->sql_query($sql);
$jelentkeztel = (int) $db->sql_fetchfield('status');
$db->sql_freeresult($result);

// vannak rajok?

$sql = " SELECT count(platoon) cnt FROM jatekszervezes_jelentk WHERE topic_id=$topic_id and platoon<>''";
$result = $db->sql_query($sql);
$rajok = (int) $db->sql_fetchfield('cnt');
$db->sql_freeresult($result);


//fekete/fehér listáról levétel
			
$fal_topic_id	= request_var('FAL1',0);
$fal_list_id	= request_var('FAL2',0);

if (($fal_topic_id>0) and ($fal_list_id>0))
{
	if ($fal_topic_id == 3217)	
	{
		$sql = "DELETE FROM jatekszervezes_feketelista WHERE list_id=$fal_list_id";
		$db->sql_query($sql);	
	}
	if ($fal_topic_id == 3216)	
	{
		$sql = "DELETE FROM jatekszervezes_feherlista WHERE list_id=$fal_list_id";
		$db->sql_query($sql);			
	}
}

//ha játékos a topik, akkor kellenek a fejlécbe extra adatok 
if ($gamedate!='')
{
	$topic_data['topic_title'] = $topic_data['topic_title'].' / '.$gamedate.', '.$battlefield.', '.$fee.' Ft, '.$sum_jel.'/'.$sum_player;
}

$sql = "select * from jatekszervezes_sites where topic_id=$topic_id"; 
$result = $db->sql_query_limit($sql, 1);
$row = $db->sql_fetchrow($result);
$site_lat		= $row['lat'];
$site_longt		= $row['longt'];
$site_tip		= $row['tipus'];
$site_email		= $row['email'];
$site_tel		= $row['telefon'];
$site_nev		= $row['nev'];
$site_met		= $row['met'];
$db->sql_freeresult($result);
// jatekszervezes_vege //
...
// jatekszervezes_eleje //
	...
	'PTIP'			=> $site_tip,
	'KONTAKT_EMAIL'	=> $site_email,
	'KONTAKT_TEL'	=> $site_tel,
	'KONTAKT_NEV'	=> $site_nev,
	'SMET'			=> $site_met,
	'P_LATITUDE'	=> $us_lat,
	'P_LONGTITUDE'	=> $us_longt, 
	'TRASMODE'		=> $trasmod, 
	...
	'STATUS'		=> $status,
	'SORT'			=> $sort,
	'SIDEIDS'		=> sizeof($sideids),

	'SZ_GAMEDATE'	=> $gamedate,
	'SZ_GAMEENDDATE'=> $gameenddate,
	'SZ_FEE'		=> $fee,
...
$sql = " SELECT " . $table_prefix . "topics.topic_title, jatekszervezes_sites.topic_id, IF(jatekszervezes_Szervez.gps IS NULL, '','selected') sel 
		 ...
		 WHERE jatekszervezes_sites.topic_id=" . $table_prefix . "topics.topic_id order by " . $table_prefix . "topics.topic_title  ";

$result = $db->sql_query($sql);

$sites_sel_info = array();
while ($row = $db->sql_fetchrow($result))
{
	$sites_sel_info[] = $row;
}
...
foreach ($sites_sel_info as $sites_select)
{
	$template->assign_block_vars('sites_select', array(
		'SITE' 		=> $sites_select['topic_title'],
		'SITE_ID' 	=> $sites_select['topic_id'],
		'SITE_SEL'	=> $sites_select['sel'])
	);
}
unset($sites_sel_info);

$sql = "SELECT count(*) cnt from jatekszervezes_Sides where topic_id = $topic_id order by side_name";
$result = $db->sql_query($sql);
$sides_cnt = (int) $db->sql_fetchfield('cnt');
$db->sql_freeresult($result);

if ($sides_cnt > 0)
{
	// oldal lista játék kiíráshoz
	...
	$sql = " SELECT side_name, side_id, side_player, concat( 'MODSIDE', @rownum := @rownum +1 ) nnr, concat( 'MODSIDE_ID', @rownum ) idnr, concat( 'MODSIDE_NR', @rownum ) pnrnr 
		     ...
		     WHERE topic_id = $topic_id order by side_name ";
	...
	foreach ($sides_info as $sides_option)
	{
		$template->assign_block_vars('sides_option', array(
			'MODSIDE' 		=> $sides_option['side_name'],
			'MODSIDE_NM' 	=> $sides_option['nnr'],
			'MODSIDE_ID' 	=> $sides_option['side_id'],
			'MODSIDE_ID_NM'	=> $sides_option['idnr'],
			'MODSIDE_NR'	=> $sides_option['side_player'],
			'MODSIDE_NR_NM'	=> $sides_option['pnrnr'])
		);
	}
	unset($sides_info);
	...
	foreach ($locksides_info as $locksides_option)
	{
		$template->assign_block_vars('locksides_option', array(
			'LOCKSIDE_NAME' 	=> $locksides_option['side_name'],
			'LOCKSIDE_ID'	 	=> $locksides_option['side_id'],
			'LOCKSIDE_STAT' 	=> $locksides_option['side_open'],
			'LOCKSIDE_STAT_NM' 	=> $locksides_option['nnr'],
			'LOCKSIDE_HIDE_NM' 	=> $locksides_option['hnnr'])
		);
	}
	unset($locksides_option);


	// oldal lista játékosoknak jelentkezéshez
	...
	$sql = " SELECT side_name, side_id, IF(side IS NULL, '','SELECTED') sel 
			 ...
			 WHERE s.topic_id = $topic_id order by side_name  ";

	$result = $db->sql_query($sql);

	$sides_sel_info = array();
	while ($row = $db->sql_fetchrow($result))
	{
		$sides_sel_info[] = $row;
	}
	...
	foreach ($sides_sel_info as $sides_select)
	{
		$template->assign_block_vars('sides_select', array(
			'SIDE' 		=> $sides_select['side_name'],
			'SIDE_ID' 	=> $sides_select['side_id'],
			'SIDE_SEL'	=> $sides_select['sel'])
		);
	}
	unset($sides_sel_info);


	
	// oldalak a jelentkezettek listáján
	...
	foreach ($sides_pl_info as $sides_pl)
	{
		$template->assign_block_vars('sides_pl', array(
			'SIDE' 		=> $sides_pl['side_name'],
			'SIDE_ID' 	=> $sides_pl['side_id'],
			'SIDE_NR' 	=> $sides_pl['side_player'])
		);
	}
	unset($sides_pl);

	// játékosok a jelentkezettek listáján
	if ( sizeof($sideids) > 0)
	{
		$sides_pl_info = array();
		$rownm = 0;
		for ($i = 0, $size = sizeof($sideids); $i < $size; $i++)
		{
			$sdpl = $sideids[$i]['side_player'];
			$sdid = $sideids[$i]['side_id'];
			$sdnm = $sideids[$i]['side_name'];
			$sdop = $sideids[$i]['side_open'];

			$sql = " SELECT (count(user_id)+sum(extra_player)) sum_jel FROM jatekszervezes_jelentk WHERE topic_id=$topic_id and side=$sdid and status=1";
			$result = $db->sql_query($sql);
			$sum_jel = (int) $db->sql_fetchfield('sum_jel');
			$db->sql_freeresult($result);

			$sz_adatok[sizeof($sz_adatok)-1]['sum_player'] = $sum_player;
			
			$db->sql_freeresult($result);

			$sides_pl_info[]['username'] = $sdnm;
			$sides_pl_info[sizeof($sides_pl_info)-1]['limiting'] = 'UJOLDAL';
			$sides_pl_info[sizeof($sides_pl_info)-1]['tcap'] = $sdpl;
			$sides_pl_info[sizeof($sides_pl_info)-1]['remark'] = $sdop;
			$sides_pl_info[sizeof($sides_pl_info)-1]['extra_player'] = $sum_jel;

			$sql = "SELECT * FROM jatekszervezes_jelentk WHERE status = 1 AND topic_id = $topic_id AND side = $sdid ORDER by signup_id asc";
			$result = $db->sql_query($sql);
			$side_cum_pl = 0;
			$side_max_signup = 0;
			...
			while ($row = $db->sql_fetchrow($result))
			{  
				$side_cum_pl = $side_cum_pl + 1 + $row['extra_player'];
				if ($side_cum_pl <= $sdpl )
				{
					$side_max_signup = $row['signup_id'] ;
				}
			}
			$db->sql_freeresult($result);
				
			$sql = "SELECT u.username, u.user_id, j.* 
				FROM jatekszervezes_jelentk j, " . USERS_TABLE . " u  
				WHERE status = 1 AND topic_id = $topic_id AND side = $sdid AND j.user_id=u.user_id 
				ORDER by platoon, u.username asc ";
			$result = $db->sql_query($sql);

			while ($row = $db->sql_fetchrow($result))
			{
				$sides_pl_info[] = $row;
				$sides_pl_info[sizeof($sides_pl_info)-1]['max_signup'] = $side_max_signup;
				$sides_pl_info[sizeof($sides_pl_info)-1]['user_rownm'] = $rownm;
				$rownm = $rownm + 1;
			}
			$db->sql_freeresult($result);
		...
		$cur_platoon = '';
		foreach ($sides_pl_info as $player_list)
		{
			if ($cur_platoon != $player_list['platoon'])
			{
				$platoon_change='Y';
			}
			else
			...
			$template->assign_block_vars('player_list', array(
				'SIDE_CH'	=> $player_list['limiting'],	
				'LIMIT' 	=> $player_list['max_signup'],	
				'SIGNUP_ID'	=> $player_list['signup_id'],
				'TEAM' 		=> $player_list['team'],
				'PLATOON' 	=> $player_list['platoon'],
				'PLATOON_CH'=> $platoon_change,	
				'WEAPM' 	=> $player_list['weap_m'],
				'FPSM' 		=> $player_list['fps_m'],
				'WEAPS' 	=> $player_list['weap_s'],
				'FPSS' 		=> $player_list['fps_s'],
				'TRANSPORT' => $player_list['transport'],
				'TFROM'		=> $player_list['tfrom'],
				'TCAP'		=> $player_list['tcap'],
				'USERLINK'	=> append_sid("{$phpbb_root_path}memberlist.$phpEx", "mode=viewprofile&amp;u=".$player_list['user_id']),	
				'REMARK'	=> $player_list['remark'],
				'EXTRA_PL'	=> $player_list['extra_player'],
				'USERNAME'	=> $player_list['username'],
				'USERID'	=> $player_list['user_id'],
				'USER_ROWNM'=> $player_list['user_rownm'],
				'REG_ACCEPT'=> $player_list['reg_accept'])
			);
			$cur_platoon = $player_list['platoon']; 
	...
	// fekete/fehér lista dolgai
if (($topic_id == 3217) or ($topic_id == 3216 ))
{
	$sql = " SELECT f.*, t.topic_title, u1.username, u2.username sendername 
			 FROM jatekszervezes_feketelista f, " . TOPICS_TABLE . " t, ". USERS_TABLE ." u1, ". USERS_TABLE ." u2 
			 WHERE t.topic_id=f.topic_id 
			 AND u1.user_id=f.user_id
			 AND u2.user_id=f.sender_id";

	if ($topic_id == 3216 )
	{
		$sql = " SELECT f.*, t.topic_title, u1.username, u2.username sendername 
				 FROM jatekszervezes_feherlista f, " . TOPICS_TABLE . " t, ". USERS_TABLE ." u1, ". USERS_TABLE ." u2 
				 WHERE t.topic_id=f.topic_id 
				 AND u1.user_id=f.user_id
				 AND u2.user_id=f.sender_id
				 ORDER BY u1.username, f.list_id";
	}
	...
	foreach ($flista_info as $fal_list)
	{
		$template->assign_block_vars('fal_list', array(
			'LIST_ID'		=> $fal_list['list_id'],
			'TOPIC_TITLE'	=> $fal_list['topic_title'],
			'SENDER_ID' 	=> $fal_list['sender_id'],
			'ADD_DATE' 		=> $fal_list['add_date'],		
			'REMARK' 		=> $fal_list['remark'],		
			'USERNAME' 		=> $fal_list['username'],	
			'SENDER_NAME' 	=> $fal_list['sendername'])
		);
	}
	unset($flista_info);
}
...
// jatekszervezes_eleje //
// első sor mindig felül
if (($forum_id == 1) or ($forum_id == 153))
{
	$sql = "select min(post_id) orig_post FROM " . POSTS_TABLE . " WHERE topic_id=$topic_id"; 
	$result = $db->sql_query($sql);
	$orig_post_id = (int) $db->sql_fetchfield('orig_post');
	$db->sql_freeresult($result);
	$post_list[] = $orig_post_id;
	for ($i = sizeof($post_list)-1; $i > 0; $i--)
	{
...
//    if (($forum_id=1) or ($forum_id=153) or ($forum_id = 26) or ($forum_id = 100) or ($forum_id = 126) or ($forum_id = 127) or ($forum_id = 128) or ($forum_id = 101) or ($forum_id = 102) or ($forum_id = 103) or ($forum_id = 104) or ($forum_id = 14) or ($forum_id = 119) or ($forum_id = 99))
	if (1==1) 
	{
		$sql = "SELECT ertek FROM jatekszervezes_hasznos WHERE post_id = ".$row['post_id'];
		$result = $db->sql_query($sql);
		$haszn_ert = (int) $db->sql_fetchfield('ertek');
		$db->sql_freeresult($result);
	}
// hasznos //
// jatekszervezes_vege //
