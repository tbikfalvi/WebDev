...
// hirdetés //
/*if (($forum_id == 97) or ($forum_id == 98) or ($forum_id == 99))
{
	$default_sort_key='s';
	$default_sort_dir='a';
}*/
...
//  szervezes
// ez a == 1 igaziból nem is kell az outer join miatt, de most maradjon
if ($forum_id == 1)
{
	$sql = 'SELECT t.topic_id  
	FROM ' . TOPICS_TABLE . " t LEFT OUTER JOIN jatekszervezes_Szervez s ON s.topic_id=t.topic_id 
	WHERE $sql_where
		AND t.topic_type IN (" . POST_NORMAL . ', ' . POST_STICKY . ")
		$sql_approved
		$sql_limit_time
	ORDER BY t.topic_type " . ((!$store_reverse) ? 'DESC' : 'ASC') . ", if(GameDate is Null, '2099-12-12', GameDate) Asc, " . $sql_sort_order;
...
// szervezes //
		...
		$gamedate	= $rowb['GameDate'];		
		$gameenddate= $rowb['GameEndDate'];		
		$playernr	= $rowb['PlayerNr'];	
		$sum_player = $rowb['PlayerNr'];	
		$sum_jel	= $rowb['jelplayernr'];		
		$fee		= $rowb['Fee'];			
		$battlefield= $rowb['BattleField'];
