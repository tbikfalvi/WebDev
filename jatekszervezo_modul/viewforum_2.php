...
$topic_tracking_info = $trackingtopics = array();
...
// szervezes //
		...
		$sql = " SELECT GameDate, Fee, BattleField FROM Szervez WHERE topic_id=$topic_id ";
		$result = $db->sql_query($sql);
		...
		$sql = " SELECT sum(side_player) sum_player FROM Sides WHERE topic_id=$topic_id and side_player>0";
		$result = $db->sql_query($sql);
		$sum_player = (int) $db->sql_fetchfield('sum_player');
		$db->sql_freeresult($result);

        $sz_adatok[0]['sum_player'] = $sum_player;
		
		$db->sql_freeresult($result);

		$sql = " SELECT (count(j.user_id)+sum(extra_player)) sum_jel FROM jatekszervezes_jelentk j, Sides s WHERE j.side=s.side_id and s.topic_id=$topic_id and s.side_player>0 and j.status=1";
		$result = $db->sql_query($sql);
		$sum_jel = (int) $db->sql_fetchfield('sum_jel');
		$db->sql_freeresult($result);

        $sz_adatok[0]['sum_jel'] = $sum_jel;
        ...
        $sz_adatok[0]['jelentkeztel'] = $jelentkeztel;
...
// szervezes //
			'SZ_GAMEDATE'				=> $sz_adatok[0]['GameDate'],
			'SZ_FEE'					=> $sz_adatok[0]['Fee'],
			'SZ_BATTLEFIELD'			=> $sz_adatok[0]['BattleField'],
			'SZ_JEL'					=> $sz_adatok[0]['sum_jel'],
			'SZ_HELY'					=> $sz_adatok[0]['sum_player'],
			'SZ_JELENTKEZTEL'			=> $sz_adatok[0]['jelentkeztel'],
// szervezes vege //
